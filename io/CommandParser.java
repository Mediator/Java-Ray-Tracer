...
import engine.forwardgraphics.renderers.GouraudShadingRenderer;
import engine.forwardgraphics.renderers.WireframeRenderer;
import engine.raytracer.renderers.RaytracerRenderer;
import engine.util.ArrayUtil;
import engine.util.Matrix;
import engine.world.Camera;
import engine.world.LightSource;
import engine.world.ModelGroup;
import engine.world.ModelMaterial;
import engine.world.World;


public class CommandParser {

	public void parse(String file, World world, HashMap<String,ModelGroup> groups)
	{
		try {
			handleFile(file, world, groups);
		} catch (Exception e) {
	...
	private void handleFile(String file, World world, HashMap<String,ModelGroup> groups) throws Exception
	{
		try {
			...
			for (int i = 0; i < len; i++) {
				...
				case 'r':
					...
					else if (longCommandName.toString().equalsIgnoreCase("recurse"))
					{
						//System.out.println("Recurse");
						if (cmd)
							throw new Exception("Invalid command format: unexpected recurse");
				...
				case 's':
					...
					while ((char)buf[++i] != ' ' && (char)buf[i] != '\t')
						longCommandName.append((char)buf[i]);
					--i;
					if (longCommandName.toString().equalsIgnoreCase("supersample"))
					{
						if (cmd)
							throw new Exception("Invalid command format: unexpected supersample");
					...
					else if (longCommandName.toString().equalsIgnoreCase("s"))
					{
						if (cmd)
				...
				default:
					...
					if (rotate)
					{
						...
						//System.out.println(Matrix.rotate(ramount, rx, ry, rz));
						//System.out.println(mg.nonPreservingMatrix);
					...
					else if (camera)
					{
						...
						if (world.cameras == null || world.cameras.length == 0)
							world.cameras = new Camera[1];
						Camera cam = null;
						for (int x = 0; x < world.cameras.length; x++)
						{
							if (world.cameras[x] == null)
								break;
							if (world.cameras[x].name.equals(curGroupName))
								cam = world.cameras[x];
						}
						if (cam == null)
						{
							cam = new Camera(curGroupName);
							if (world.cameras.length > 1)
								world.cameras = ArrayUtil.redimArray(world.cameras, world.cameras.length+1);

							world.cameras[world.cameras.length-1] = cam;
						}
					...
					else if (wireframe)
					{
						...
						if (wCam == null)
						{
							...
							if (world.cameras.length > 1)
								world.cameras = ArrayUtil.redimArray(world.cameras, world.cameras.length+1);
							world.cameras[world.cameras.length-1] = wCam;
						}
						world.initializeGraphics(wCam.name, minX, minY, maxX, maxY);
						world.renderer = new WireframeRenderer();
					...
					else if (material)
					{
						...
						while (i < buf.length && (char)buf[i] != '\r' && (char)buf[i] != '\n')
						{
							...
							if (i >= buf.length || (char)buf[i] == ' ' || (char)buf[i] == '\r' || (char)buf[i] == '\n' || (char)buf[i] == '\t')
							{
								...
								{
									if (!bld.toString().trim().equals(""))
									{
										matTrans = Double.parseDouble(bld.toString());
										bld = new StringBuilder();
									}
								}
								if (i >= buf.length || (char)buf[i] == '\r' || (char)buf[i] == '\n')
						...
						ModelMaterial m = World.materials.get(curGroupName);
						if (m == null)
						{
							m = new ModelMaterial(curGroupName);
							World.materials.put(curGroupName, m);
						}
						m.setRGB(colorR, colorG, colorB);
						m.setSpecular(matSpec);
						m.setReflectance(matReflect);
						m.setTransparency(matTrans);
					...
					else if (light)
					{
						...
						World.lights.add(new LightSource(lightX, lightY, lightZ, lightW, lightR, lightG, lightB));
						light = false;
						cmd = false;
					}
					else if (supersample)
					{
						...
						boolean needsStart = true, needsEnd = true, needsInc = true;
						double SSStart = 0, SSEnd = 0, SSInc = 0;
						while (i < buf.length && (char)buf[i] != '\r' && (char)buf[i] != '\n')
						{
							if ((char)buf[i] == ' ')
							{
							...
							if (i >= buf.length || (char)buf[i] == ' ' || (char)buf[i] == '\r' || (char)buf[i] == '\n' || (char)buf[i] == '\t')
							{
								if (needsStart == true)
								{
									SSStart = Double.parseDouble(bld.toString());
									bld = new StringBuilder();
									needsStart = false;
								}
								else if (needsEnd == true)
								{
									SSEnd = Double.parseDouble(bld.toString());
									bld = new StringBuilder();
									needsEnd = false;
								}
								else if (needsInc == true)
								{
									SSInc = Double.parseDouble(bld.toString());
									bld = new StringBuilder();
									needsInc = false;
								}
								if (i >= buf.length || (char)buf[i] == '\r' || (char)buf[i] == '\n')
								{
									if (needsInc == true)
										throw new Exception("Invalid command format: supersample missing arguement");
								}
							}
						}
						world.superSamplingStart = SSStart;
						world.superSamplingEnd = SSEnd;
						world.superSamplingIncrement = SSInc;
						world.superSample = true;
						supersample = false;
						cmd = false;
					}
					else if (recurse)
					{
						...
						while (i < buf.length && (char)buf[i] != '\r' && (char)buf[i] != '\n')
						{
							...
							if (i >= buf.length || (char)buf[i] == ' ' || (char)buf[i] == '\r' || (char)buf[i] == '\n' || (char)buf[i] == '\t')
							{
								if (needsR == true)
								{
									rVal = Integer.parseInt(bld.toString());
									bld = new StringBuilder();
									needsR = false;
								}
								if (i >= buf.length || (char)buf[i] == '\r' || (char)buf[i] == '\n')
								{
									if (needsR == true)
										throw new Exception("Invalid command format: recurse missing arguement");
								}
							}
						}
						world.recursiveDepth = rVal;
						recurse = false;
						cmd = false;
					}
					else if (render)
					{
						...
						world.initializeGraphics(wCam.name,minX, minY, maxX, maxY);
						world.renderer = new GouraudShadingRenderer();
						render = false;
						cmd = false;
					}
					else if (raytrace)
					{
						...
						world.initializeGraphics(wCam.name,minX, minY, maxX, maxY);
						world.renderer = new RaytracerRenderer();
						world.renderingRequiresAllGroups = true;
						raytrace = false;
						cmd = false;
					}
					else if (comment)
