...
public class ModelParser {
	...
	private HashMap<String,ModelGroup> handleFile(String file) throws Exception
	{
		...
		try {
			...
			curMaterial.setRGB(0, 0, 0);
			curMaterial.setReflectance(1);
			curMaterial.setSpecular(.1);
			...
			for (int i = 0; i < len; i++) {
				...
				default:
					...
					if (material)
					{
						...
						{
							if (World.materials.containsKey(bld.toString()))
							{
								curMaterial = World.materials.get(bld.toString());
							}
							else
							{
								curMaterial = new ModelMaterial(bld.toString());
								World.materials.put(bld.toString(),curMaterial);
							}
							bld = new StringBuilder();
						}
						material = false;
