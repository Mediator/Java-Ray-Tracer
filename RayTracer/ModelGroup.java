	...
	public boolean validate()
	{
		...
		for (int x = 0; x < polys.length;x++)
		{
			...
			for (int y = 3; y < polys[x].myVerts.length;y++)
			{
				...
				double ret = Matrix.dotProduct(normalV,v3);
				...
				double ret2 = Matrix.dotProduct(normalV, normal2);
...
/*
		...
		*/
		...
		double[] fpNormal = this.polys[0].normal;
		...
		if (this.polys.length != totalPolys)
		{
			...
			int vertPos = originalVertLength;
			...
			for (int pp = 0; pp < polys.length; pp++)
			{
				for (int pv = 0; pv < 3; pv++)
				{
					if (workVerts[vv] == polys[pp].myVerts[pv])
					{
						...
						{
							if (Matrix.dotProduct(fNormal,polys[pp].normal) < 0)
							{
	...
	/*public void applyOpertation(Camera cam, WorkNotifier notifier)
	{
		this.notifier = notifier;
		Matrix workingMatrix = Matrix.multiply(cam.cameraMatrix, this.composedMatrix);
		//System.out.println(workingMatrix);
		Polygon.composedVerts = new double[Polygon.verts.length][4];
		...
		for (int x = 0; x < WorkPool.cores; x++)
		{
			...
			TransformationWorker z = new TransformationWorker(workingMatrix,workVerts,x*tasksPerThread,tstop);
			workPool.addWork(z);
	...
	public void applyOpertation(Camera cam, WorkNotifier notifier, boolean zbuffer)
	{
		...
		if (!cam.raytrace)
		{
			workingMatrix = Matrix.multiply(cam.cameraCoordsMatrix, this.composedMatrix);
			for (int x = 0; x < lights.size();x++)
			{
				lights.get(x).composedLocation = Matrix.multiply(cam.cameraCoordsMatrix, lights.get(x).location);                            
				// lights.get(x).composedLocation = Matrix.multiply(lights.get(x).composedLocation, -1);
			}
		}
		else
		{
			workingMatrix = this.composedMatrix;
		}
		...
		for (int x = 0; x < WorkPool.cores; x++)
		{
			...
			ProjectionTransformationWorker z = new ProjectionTransformationWorker(cam, workingMatrix,cam.projectionMatrix, nonPreservingMatrix,workVerts,x*tasksPerThread,tstop);
			workPool.addWork(z);
	...
	public void notified() {
		...
		if (cnt == 0)
		{
			...
			/*if (Polygon.maxDepth < 0)
			{
			Polygon.zShift = Polygon.maxDepth * -1 + 100;
			}
			else
			Polygon.zShift = 100;*/
