	...
	private void handleModel(String file, HashMap<String,ModelGroup> groups) throws Exception
	{
		...
		for (int x = 0; x < ngroups.length; x++)
		{
			...
			try
			{
				...
				if (!outputtedVerts)
				{
					for (int z = 0; z < Polygon.composedVerts.length; z++)
					{
						out.write("v " + Polygon.composedVerts[z][0] + " " + Polygon.composedVerts[z][1] + " " + Polygon.composedVerts[z][2] + " " + Polygon.composedVerts[z][3] + "\n");
					}
					outputtedVerts = true;
				}
