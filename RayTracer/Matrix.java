...
public class Matrix {
    ...
    public static  double[] solve(Matrix lhs,double[] rhs) {
        ...
        for (int i = 0; i < lhs.values[0].length; i++) {
            ...
            for (int j = i + 1; j < lhs.values[0].length; j++)
                if (Math.abs(lhs.values[j][i]) > Math.abs(lhs.values[max][i]))
                    max = j;
            lhs.swap(i, max);
	...
	public static Matrix rotate(double theta, double xAxis, double yAxis, double zAxis)
	{
		...
		double minAxium = Math.min(Math.abs(w[2]), Math.min(Math.abs(w[0]), Math.abs(w[1])));
		...
		if (minAxium == Math.abs(u[0]))
		{
			u[0] = 1;
		}
		else if (minAxium == Math.abs(u[1]))
		{
			u[1] = 1;
		}
		else
