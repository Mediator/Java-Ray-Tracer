...
public class Polygon {
	...
	public static double[][] projectedVerts;
	public int[] myVerts;
	...
	{
		/*try {
			texture = ImageIO.read(new File("texture.gif"));
		} catch (IOException e) {
	...
	public String toString()
	{
		...
		sb.append(Arrays.toString(transformedVerts[myVerts[0]]) + ", " + Arrays.toString(transformedVerts[myVerts[0]]) + " " + Arrays.toString(transformedVerts[myVerts[2]]) + " " + this.hashCode());
		return sb.toString();
	}
...
/*	public Polygon trianglify(int start)
	...
	public void colorify(World world)
	{
		...
		for (int vx = 0; vx < 3; vx++)
		{
			...
			for (int x = 0; x < World.lights.size(); x++)
			{
				...
				if (Matrix.dotProduct4(composedVertNormals[vertIndex], V) < .1)
				{
					V[0] *= -1;
					V[1] *= -1;
					V[2] *= -1;
				...
				NdotL = Matrix.dotProduct4(composedVertNormals[vertIndex],L);
